...
| `assets/mark.svg` | 品牌标记（黄绿方块 + 对角线留白，呼应不等式「合」的理念） |
...
4. **不把内部材料发布出去**：任何源自 App 仓库草案的内容必须过 `publish_clean`（内部说明块、附录、占位项、任务卡号/复审号/决策号、`docs/` 路径、"主人"称谓、**内部口径术语如「豁免口径/豁免披露」**）。
...
5. **联系方式单一事实源**：公司名/地址/邮箱/电话在 `tools/build_legal.py` 顶部 `COMPANY`（法律页）与 4 个营销页中保持一致；三处对齐：本站 · `IOS-TREP/docs/104` A1.5 · ICP 备案与 ASC 信息。
6. **ICP 备案号是法定义务**：备案通过后必须把 7 页页脚「ICP 备案：审核中」替换为真实备案号并链接 `https://beian.miit.gov.cn/`。
7. **官网资产不放进 App 仓库**（主人 2026-09-17 明确要求）；App 侧只在 `backend/deploy/nginx.conf` 保留"如何托管本静态站"的注释与 `backend/site-dist/` 部署约定。
8. **密钥/账号零入库**：本站无任何密钥；GitHub 仓库必须 **public**（免费账号 Pages 仅支持公开仓库）。
...
9. **GitHub 会自动往本仓库提交**（改 Pages 自定义域 → `Create/Delete CNAME`；域名/证书同步 → `Update CNAME`）：推送前先 `git fetch`（`tools/push.sh` 已自动做）；解冲突时**先确认文件里没有 `<<<<<<<` 标记**再 `git add`（本轮曾把冲突标记提交进 `CNAME`，靠 GitHub 自动纠正，不可依赖）。
10. **`github.com` 直连在国内常超时**（`api.github.com` 仍通）：`tools/push.sh` 会探测并自动改走本机代理（`PROXY_URL`，默认 `http://127.0.0.1:7897`），也可 `GIT_TRANSPORT=direct|proxy` 强制。
11. **Pages API 设自定义域不要同时带 `https_enforced`**（会报 `The certificate does not exist yet`）：先 `PUT {"cname":"..."}`（204）→ 等证书 `approved`。
