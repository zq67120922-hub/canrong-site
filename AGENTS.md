...
1. **版式语言（v3，2026-09-17 按 Hallmark 重做）**：**左对齐编辑式版式**，不用「全站居中 + 卡片墙 + 等距」。
   硬性要求：
   · 布局有主轴（左偏），**除首页 Hero 外一律不居中**；章节标题与正文左对齐成组出现；
   · **禁止卡片/面板**：区块之间用 **1px 细分隔线**（`.rows` / `.spec` / `.endnote`）与留白组织；不做卡中卡；
   · **间距不再处处相等**：用 4pt 刻度九档（`--s-3xs … --s-3xl`），同页混用大小档；`--section-gap` 一律不写固定 px；
   · **字号按 perfect fourth（1.333）**：`--t-xs … --t-3xl`；正文 ≥14px；
   · **字体三族为上限**：正文系统无衬线（`--font-body`）· 显示用中文衬线（`--font-display`，章节/页标题与 `.pull`）· 数字与微标用等宽（`--font-mono`，仅「编号/日期/标签」两个角色）；**禁止渐变标题、禁止把三个槽位都用满**；
   · **禁止眉标**（`kicker` 这类「小圆点 + 一行字」贴在标题上方）——Hallmark gate 54 的典型模板味；微标只允许出现在**正文级**位置（`.note` / `.mono`）；
   · 一个页面最多一处破格元素（`.pull` / 通栏色块 / 波形带）；**颜色一律走令牌，页面内联色值必须为 0**。
2. **业务线落地页各用一种结构**（docs/05 §二，Hallmark 结构指纹；**抽掉颜色，四页应像四个不同的站**）：
